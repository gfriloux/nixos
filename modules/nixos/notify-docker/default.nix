{
  ...
  notifyScript = pkgs.writeShellScript "notify-failure" ''
    ...
    ${pkgs.curl}/bin/curl -s \
      -H "Title: [${host}] $SERVICE en défaut" \
  ...
  watched =
    lib.filterAttrs (
      _: c: (c.labels or {}) ? "friloux.me/health-watch"
    )
    config.virtualisation.oci-containers.containers;
in {
  options.kuri.notify-docker.enable = lib.mkEnableOption "Docker container failure notifications";
